      ...
      <div id="infoContent">
        ...
        <div>
          <b-button id="updateBtn" variant="danger" @click="updateCheck"
            >수정</b-button
          >
          <p id="deleteBtn" @click="$bvModal.show('bv-modal-example')">
            회원탈퇴
          </p>
          <b-modal id="bv-modal-example" hide-footer>
            ...
            <b-button id="goOutBtn" class="mt-3" block @click="memberDelete"
              >회원 탈퇴</b-button
            >
            <b-button
              class="mt-3"
  ...
  methods: {
    ...
    showImg() {
      this.user.profileImg =
        "https://i4a406.p.ssafy.io:5000/imgs/getByteImage/" + this.user.userid;
    },
      ...
      if (this.user.userpwd == this.user.userpwdconfirm) {
        ...
        if (document.getElementById("upload").files[0]) {
          if (this.checkimg.includes("kakao")) {
            ...
            axios
              .post(`https://i4a406.p.ssafy.io:5000/imgs/saveImage`, fd)
              .then();
          } else {
            const fd = new FormData();
            fd.append("upload", document.getElementById("upload").files[0]);
            axios
              .put(
                `https://i4a406.p.ssafy.io:5000/imgs/updateImage/${this.user.userid}`,
                fd,
              )
              .then();
          }
