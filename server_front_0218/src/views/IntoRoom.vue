      ...
      <div :class="[{ fullscreen: chatToggle }, 'left_layout']">
        ...
        <div class="roomContents">
          <div class="cameraContainer" v-if="accepted">
            <div v-if="!privateScreenShareCheck">
              <user-video
                class="userVideo"
          ...
          <div id="chatContainer">
            ...
            <div class="chatmenu">
                  ...
                  <ul>
                    ...
                    <li v-else>
                      <span
                        ...
                        >마피아 게임</span
                      >
                      <span v-else id="mafiaNo" class="gameContent"
                        >마피아 게임(4인이상)</span
                      >
                    </li>
                    <li>
              ...
              <div class="chatmenu_off" v-show="this.user.manager === 1">
                ...
                <span class="tooltiptext">입장수락</span>
              </div>
...
export default {
  ...
  methods: {
    ...
    getRandomGame() {
      axios
        .get(`https://i4a406.p.ssafy.io:5000/games/recommend`)
        .then((response) => {
          const rGame = response.data;
          this.publisher.session.signal({
            data: JSON.stringify({
              message: "=====당신이 뽑은 게임은! ===== 아래 확인!",
            }),
            type: "chat",
          });
    ...
    liarGame() {
      ...
      axios
        .get(`https://i4a406.p.ssafy.io:5000/games/liar`)
        .then((response) => {
    ...
    getTopic(level) {
      axios
        .get(`https://i4a406.p.ssafy.io:5000/games/topic/${level}`)
        .then((response) => {
          const topic = response.data;
          this.publisher.session.signal({
            data: JSON.stringify({
              message: "=====당신이 뽑은 주제는===== 아래 확인!",
            }),
            type: "chat",
          });
    ...
    getEndMafia() {
      this.session.on("signal:endMafia", () => {
        this.mafia = "";
      });
    },
    ...
    joinSession(session_id) {
      ...
      axios // add
        .post(`https://i4a406.p.ssafy.io:5000/user/info`)
        .then((response) => {
          this.myUserName = response.data.user.username;
          axios // add
            .get(`https://i4a406.p.ssafy.io:5000/room/selectroom/${session_id}`)
            .then((res) => {
              if (res.data.message === "FAIL") {
                ...
                axios // add
                  .put(
                    `https://i4a406.p.ssafy.io:5000/room/updateRoom/${session_id}`,
                    this.room,
                  )
                  .then();
              }
      ...
      axios
        .get(`https://i4a406.p.ssafy.io:5000/session/maketoken/${session_id}`)
        .then((response) => {
    ...
    leaveSession() {
      ...
      if (this.user.manager === 1) {
        this.publisher.session.signal({
          data: JSON.stringify({
            message:
              "=====방장이 나갔습니다. 몇몇 기능이 안될 수 있습니다. 더 많은 기능을 즐기기 위해 다른 방으로 이동해주세요.=====",
          }),
          type: "chat",
        });
      }
      ...
      axios
        .post(`https://i4a406.p.ssafy.io:5000/session/removeUser`, {
    ...
    screenShare() {
      this.privateScreenShareCheck = true;
      // --- Get an OpenVidu object ---
      ...
      axios
        .get(
          `https://i4a406.p.ssafy.io:5000/session/maketoken/${this.session_id}`,
        )
        .then((response) => {
    ...
    stopScreenShare() {
      ...
      this.privateScreenShareCheck = false;
  ...
  computed: {
    mafiaAble: function () {
      return this.subscribers.length >= 3;
    },
  },
};
</script>
...
#games > ul > li:hover {
  height: 30px;
  width: 130px;
  text-decoration: rgba(0, 255, 0, 0.404);
  text-decoration-line: underline;
}
#backg2 {
  visibility: hidden;
